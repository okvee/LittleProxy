...
public class HttpProxyTest {
    ...
    public static final int PROXY_PORT = 1081;
    ...
    public void testProxyChunkAndNo() throws Exception {
        ...
        try {
            ...
            final byte[] proxyResponse = rawResponse("127.0.0.1", PROXY_PORT, false, HttpVersion.HTTP_1_1);
    ...
    private String httpPostWithApacheClient(final boolean isProxy, 
        ...
        try {
            if (isProxy) {
                final HttpHost proxy = new HttpHost("127.0.0.1", PROXY_PORT);
                httpclient.getParams().setParameter(ConnRoutePNames.DEFAULT_PROXY, proxy);
                if(username != null && password != null){
                    httpclient.getCredentialsProvider().setCredentials(
                        new AuthScope("127.0.0.1", PROXY_PORT), 
    ...
    private String httpGetWithApacheClient(boolean isProxy, String username, 
        ...
        try {
            if (isProxy) {
                HttpHost proxy = new HttpHost("127.0.0.1", PROXY_PORT);
                httpclient.getParams().setParameter(ConnRoutePNames.DEFAULT_PROXY, proxy);
                if(username != null && password != null){
                    httpclient.getCredentialsProvider().setCredentials(new AuthScope("127.0.0.1", PROXY_PORT), 
                        new UsernamePasswordCredentials(username, password));
                }
            }
    ...
    private HttpProxyServer startHttpProxyWithCredentials(final String userName, 
        ...
        final HttpProxyServer server = new DefaultHttpProxyServer(PROXY_PORT);
